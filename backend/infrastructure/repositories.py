...
class IDailyStatRepository(Protocol):
    ...
    def get_stat_by_date(self, user_id: str,
                         target_date: date) -> DailyStatOrm | None: ...
    def get_stats_for_user(
        self, user_id: str, limit: int = 7) -> list[DailyStatOrm]: ...
...
class IRecipeRepository(Protocol):
    ...
    def create_recipe_with_steps(
        self, recipe_data: dict, steps_data: list[dict]) -> RecipeOrm: ...
